...
export class AjaxService {
  ...
  public post(config) {
    ...
    config.success = function (result, status, xhr) {
      ...
      //过滤登录
      if (
        // xhr.getResponseHeader("serverError") ||
        xhr.getResponseHeader("serverError") === "sessionOut") {
        if (this.router) this.router.navigate(['/pages/login'], {replaceUrl: true}); //路由跳转
      } else {
